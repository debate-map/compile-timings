...
fn extract_units_data(content: &str) -> UnitsBuildData {
    ...
    let row_re =
        Regex::new(r#"<tr>\s*<td>\d+\.</td>\s*<td>(.*?)</td>\s*<td>(\d+(?:\.\d+)?s)</td>"#)
            .unwrap();
    ...
    table_re
        ...
        .map(|table| {
            row_re
                ...
                .map(|row| UnitBuildData {
                    name: row[1].to_string(),
                    time: row[2].trim_end_matches('s').parse().unwrap(),
                })
                .collect()
        })
        .unwrap_or_default()
...
fn ensure_tracker_file_exists(path: &Path) -> Result<()> {
    if !path.exists() {
...
fn main() -> Result<()> {
    ...
    let build_metadata = BuildMetadata {
        total_time: extract_value(&html_content, r"<td>Total time:</td><td>(\d+(?:\.\d+)?)s")
            .unwrap()
            .parse()?,
        rustc_version: extract_value(&html_content, r"<td>rustc:</td><td>(rustc [\d\.\w-]+)")
            .unwrap(),
        total_units: extract_value(&html_content, r"<td>Total units:</td><td>(\d+)")
            .unwrap()
            .parse()
            .unwrap(),
        build_start: extract_value(input_filename, r"(\d{8}T\d{6}Z)").unwrap(),
    };

    let units_data = extract_units_data(&html_content);

    write_json_file(
        &metadata_dir.join(format!("{}.json", build_metadata.build_start)),
        &build_metadata,
    )?;
    write_json_file(
        &units_dir.join(format!("{}.json", build_metadata.build_start)),
        &units_data,
    )?;
